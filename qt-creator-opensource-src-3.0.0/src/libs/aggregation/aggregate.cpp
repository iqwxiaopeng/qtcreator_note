...
/*!
	...
	聚合类定义了一个相关组件的集合，这些组件可以看作一个单元。
	聚合是作为一个单元处理的组件的集合，
	使每个组件向外部公开聚合中其他组件的属性和行为。
	具体来说，这意味着：
	\li 它们可以相互“转换”（使用查询和查询所有函数）。
	\li 它们的生命周期是耦合的，即当一个被删除时，集合所有的其他都会被删除。
	组件可以是任何qobject派生类型。


    You can use an Aggregate to simulate multiple inheritance by aggregation. Assume we have
	可以使用聚合通过聚合模拟多重继承。假设我们有
    \code
	...
	如果我们希望“object”也实现myinterfaceex类，
	但不想或不能使用多重继承，我们可以在任何时候使用聚合：
	...
	支持聚合的代码从不使用qobject_cast，而是始终使用
...
/*!
	...
	返回具有给定类型（如果有）的组件的模板函数。
	如果有多个具有该类型的组件，则返回随机组件。
...
/*!
	...
	创建具有给定父级的新聚合。
	父类被直接传递给类的qobject部分，并且不在该部分旁边使用。
