...
/*
mime database类是一个mime类型的数据库，插件可以将它们处理的mime类型添加到该数据库中。
类受qmutex保护，因此可以由线程访问。
*/
...
public:
    ...
    static QStringList filterStrings();//unknown (*.a *.dll *.exe *.lib *.o *.obj *.out *.so) 过滤文件类型的字符串
