...
QAction *ActionContainerPrivate::insertLocation(QList<Group>::const_iterator group) const
{
    ...
    while (group != m_groups.constEnd()) {
        if (!group->items.isEmpty()) {
			//只对第一条做操作,发现一个cmd或者menu则返回
            QObject *item = group->items.first();
			//因为group并非仅仅存放 ActionContainer因此要判断处理
            if (Command *cmd = qobject_cast<Command *>(item)) { 
                return cmd->action();
            } else if (ActionContainer *container = qobject_cast<ActionContainer *>(item)) {
                if (container->menu()) //包含Qmenu对象
                    return container->menu()->menuAction(); //返回Qmenu绑定的QAction
            }
...
//向容器集中某个组中添加一个菜单,容器中的每个条目都是一个组来表示的
void ActionContainerPrivate::addMenu(ActionContainer *menu, Id groupId)
{
    ...
    if (!containerPrivate->canBeAddedToMenu()) //先判断是否可以添加到menu,因为只有代表menu的容器能被添加到menu
        return;

    MenuActionContainer *container = static_cast<MenuActionContainer *>(containerPrivate); //可被添加证明是一个menu容器
    const Id actualGroupId = groupId.isValid() ? groupId : Id(Constants::G_DEFAULT_TWO); //获取要添加到的组的id
    QList<Group>::const_iterator groupIt = findGroup(actualGroupId); //根据id从容器条目集中取得对应条目的迭代器对象
    QTC_ASSERT(groupIt != m_groups.constEnd(), return); //没找到直接返回
    QAction *beforeAction = insertLocation(groupIt);
    m_groups[groupIt-m_groups.constBegin()].items.append(menu); //由指针偏移量得到索引

    connect(menu, SIGNAL(destroyed()), this, SLOT(itemDestroyed()));
    insertMenu(beforeAction, container->menu());//调用menuBar的insertMenu
    scheduleUpdate();
}
...
{
    static int separatorIdCount = 0;//静态 每个分隔符ID唯一
    QAction *separator = new QAction(this);
    separator->setSeparator(true); //把一个QAction作为分隔符
