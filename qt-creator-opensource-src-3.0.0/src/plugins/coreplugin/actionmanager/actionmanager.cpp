...
Command *ActionManager::registerShortcut(QShortcut *shortcut, Id id, const Context &context, bool scriptable)
{
	...
	shortcut->setContext(Qt::ApplicationShortcut); //此属性保存快捷方式有效的上下文。快捷方式的上下文决定允许在哪种情况下触发快捷方式。
    ...
    if (isPresentationModeEnabled()) //如果开启演示模式
...
{
    if (enabled == isPresentationModeEnabled()) //设置没有变更则返回
        return;

    // Signal/slots to commands: 演示模式开启 则把所有的菜单操作和 快捷方式触发绑定到 actionTriggered 与 shortcutTriggered
    ...
    // The label for the shortcuts: 如果演示的label没有创建 则创建
