...
#if defined(Q_OS_WIN)
	//如果想知道当前进程在哪个终端服务(Terminal service)会话中运行，可以借助于ProcessIdToSessionId函数
    ...
    : QObject(parent), id(appId)
{
	...
	//获取一个与进程相关的id值 作为套接字名
    socketName = appSessionId(id);
    server = new QLocalServer(this);//创建一个服务器
...
bool QtLocalPeer::isClient()
{
    ...
    bool res = server->listen(socketName); //开启监听 监听某个服务的名字 这里怀疑并非正常的套接字 只是抽象了套接字通讯
    if (!res)
        qWarning("QtSingleCoreApplication: listen on local socket failed, %s", qPrintable(server->errorString()));
    QObject::connect(server, SIGNAL(newConnection()), SLOT(receiveConnection())); //绑定accept回调 有连接过来
    return false;
}
...
void QtLocalPeer::receiveConnection()
{
    ...
    quint32 remaining;
    //ds >> remaining;
    socket->write((char*)&remaining, sizeof(quint32));
    ...
    do {
       // got = ds.readRawData(uMsgBuf, remaining);
        got = socket->read(uMsgBuf, remaining);
        remaining -= got;
        uMsgBuf += got;
