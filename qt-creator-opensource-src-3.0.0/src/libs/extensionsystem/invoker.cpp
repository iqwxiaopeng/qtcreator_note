...
void InvokerBase::invoke(QObject *t, const char *slot)
{
    ...
    sig.append(')');
    sig.append('\0'); //sig中是 方法名(参数1，参数2，参数3...)
    int idx = target->metaObject()->indexOfMethod(sig.constData()); //获取方法所在元数据中索引
    if (idx < 0)
        return;
    QMetaMethod method = target->metaObject()->method(idx); //根据索引拿到方法指针
    if (useRet) //有返回值则穿参调用接收返回
