...
void FancyToolButton::paintEvent(QPaintEvent *event)
{
    ...
    if (!Utils::HostOsInfo::isMacHost() // Mac UIs usually don't hover
        ...
        painter.restore();
    } else if (isDown() || isChecked()) { //按钮被按下或者选中
        painter.save();
        QLinearGradient grad(rect().topLeft(), rect().topRight());
        grad.setColorAt(0, Qt::transparent);
    ...
    // draw popup texts
    if (isTitledAction) { //对于titleAction不为空的 绘制弹出文字
...
{
    QSizeF buttonSize = iconSize().expandedTo(QSize(64, 38)); //64*38图标大小
    if (defaultAction()->property("titledAction").toBool()) { //存在titledAction
        QFont boldFont(font());
        boldFont.setPointSizeF(Utils::StyleHelper::sidebarFontSize());
        boldFont.setBold(true);
        QFontMetrics fm(boldFont);
        qreal lineHeight = fm.height(); //拿到上面求的字体高度
        const QString projectName = defaultAction()->property("heading").toString();
        //有heading 属性 高+10  否则高+2
        buttonSize += QSizeF(0, 10);
        if (!projectName.isEmpty())
            buttonSize += QSizeF(0, lineHeight + 2);

        buttonSize += QSizeF(0, lineHeight*2 + 2); //最终大小
    }
